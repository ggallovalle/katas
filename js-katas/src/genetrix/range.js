"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.range = exports.InfiniteLoopError = void 0;
var InfiniteLoopError = /** @class */ (function (_super) {
    __extends(InfiniteLoopError, _super);
    function InfiniteLoopError() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.name = "InfiniteLoopError";
        return _this;
    }
    return InfiniteLoopError;
}(Error));
exports.InfiniteLoopError = InfiniteLoopError;
function range(start, stop, step) {
    var index, index;
    if (step === void 0) { step = 1; }
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (stop == null) {
                    stop = start;
                    start = 0;
                }
                if (!(step > 0)) return [3 /*break*/, 5];
                if (start > stop)
                    throw new InfiniteLoopError("`start` > `stop` in incrementing counter");
                index = start;
                _a.label = 1;
            case 1:
                if (!(index <= stop)) return [3 /*break*/, 4];
                return [4 /*yield*/, index];
            case 2:
                _a.sent();
                _a.label = 3;
            case 3:
                index += step;
                return [3 /*break*/, 1];
            case 4: return [3 /*break*/, 11];
            case 5:
                if (!(step < 0)) return [3 /*break*/, 10];
                if (start < stop)
                    throw new InfiniteLoopError("`start` < `stop` in decrementing counter");
                index = start;
                _a.label = 6;
            case 6:
                if (!(index >= stop)) return [3 /*break*/, 9];
                return [4 /*yield*/, index];
            case 7:
                _a.sent();
                _a.label = 8;
            case 8:
                index += step;
                return [3 /*break*/, 6];
            case 9: return [3 /*break*/, 11];
            case 10: throw new InfiniteLoopError("With a `step=0` you will have an infinite loop");
            case 11: return [2 /*return*/];
        }
    });
}
exports.range = range;
// for (const x of takingAtATime(2, count(5))) {
//   console.log(x)
// }
